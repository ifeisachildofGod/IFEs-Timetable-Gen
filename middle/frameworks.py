...
# Not yet ready
# def text_from_school(school: SchoolFrameWork):
#     text = ""
...
#     text += "\n_\n".join([" ; ".join([" ".join([("0" if s.id == FreePeriodFW.id else ("-1" if s.id == BreakPeriodFW.id else str(list(c.subjects).index(s.id) + 1))) for s in l_subj]) for l_subj in c.timetable.values()]) for c in school.classes.values()]) + "\n" # type: ignore
